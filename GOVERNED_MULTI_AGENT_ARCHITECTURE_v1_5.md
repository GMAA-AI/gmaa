...
> **U-8** Seat surfaces: the chat-seat archetype and operator-ferry transport (omission repair — day-one practice in every origin program; the doc referenced "non-chat seats" without ever defining the chat surface, though the adoption path and all three running programs depend on it). → §1.1, §5. Found while chartering the third program's chat seat.
>
> **Revision note (v1.2 → v1.3):**
> **U-7** Boot-0, the governed spawn (omission repair — origin-program session law: the ordered procedure between seat spawn and first work item; the doc carried the restart-cheap principle without the boot procedure that implements it). Found by the same first Code-seat spawn as U-6. → §7.
>
> **Revision note (v1.1 → v1.2):**
> **U-6** Seat runtime substrate + doorbell law (omission repair — origin-program practice: persistent named multiplexer sessions [tmux reference], durable/addressable/attachable/observable properties, doorbell carries attention only) → §7. Found by a peer program's first Code-seat spawn: the doc referenced the doorbell and named session slots without ever specifying the session layer beneath them.
>
> **Revision note (v1.0 → v1.1, one ratified set — RULING XP-R1 + architect ratification of the omission-repair class):**
> **U-1** LIVE-DATA-ONLY declared-substrate ruling (omission repair — origin-program day-one law dropped in extraction; independently converged by the a peer program instantiation §1.3) → §2.
> **U-2** Conformed multi-program evidence schema incl. greenfield `PENDING first HEAD` convention (contribution, a peer program; ratified XP-R1) → §4.
> **U-3** Completeness firewall (omission repair — day-one thesis law) → §13.
> **U-4** Subagent doctrine (omission repair — origin-program ruled law: role-class taxonomy with hard constraints, role-task match, bounded ratified roster, loadability verification, intra-seat-only rule, fan-out rules, cost envelope) → §1.3.
> **U-5** Session lifecycle / restart-cheap doctrine (omission repair — origin-program ledgered law, evidence class E-005) → §3 rule 8.
...
**v1.5 — 2026-07-27.** Ratified set (GMAA architect seat; HOW-class per XP-R1 rev-2 precedent; commits at foundation under the XP-05 grant): **U-14** chat/Code-session role assessment → §1.1 (contribution, operator-directed; roles derived from the custody split per origin strengthening) · **U-15** coherence-session law → §2 fifth universal invariant + §3 rule 9 + §7 generation line (prohibition/census: omission repair, founding thesis law, a peer program C-4 concurred; reasoning-shape bindings: contribution, a peer program incident-accounting provenance) · **U-16** seat-creation provisioning law → §7 (sequence: omission repair, origin charter-precedes-boot law; launcher-map defect repair named) · **U-17** program-qualified session identity → §7 (contribution; XP-R2 token registry reused; doorbell-R1 extension relayed for origin concurrence) · **U-18** definitions block → §1.0 (omission repair; operating law made explicit) · **U-19** Boot-0 §7 body (omission repair closing the v1.3 fake-closure defect; EXTRACTED from held production texts — origin launcher v2, stamped BOOTSTRAP, CHARTER-foundation, package CLAUDE.md — per the operator-ruled extraction method; one step bracketed pending foundation verbatim supply) · **U-20** observed-mode boot receipt → §8 (omission repair; law text extracted from origin launcher header; ruling cites an origin ruling / an origin ruling; incident evidence lands separately at the origin ledger) · **U-21** liveness & unresponsiveness doctrine → §7 (omission repair of distributed origin practice, receipt-confirmation pending at foundation flagged inline) · **C-3** intra-program relay convention → §5 (contribution, a peer program). Evidence-grade schema (C-1) proceeds as XP-R1 §3 revision at the origin custodian seat, referenced in Appendix A. FIX-SURFACE-DIVERGENCE enters Appendix A as a named class (origin claims-lane naming act, 2026-07-26).
...
Class rules, all ruled law in the origin program:
...
- **Cost envelope (ruled, not assumed).** Subagent fan-out multiplies token cost — order-of-magnitude multipliers are verified in the origin program's receipts, not hypothetical. Each instantiation rules a cost envelope for subagent-heavy flows (model floor and/or batch limits) as an invariant with receipts. Uncontrolled cost is a silent-degradation vector: it pressures exactly the shortcuts (skipped validation, smoke bypasses, mode escalation) the invariants prohibit. Pricing the boundary's overhead honestly is part of proving it viable.
...
**Intra-program channels.** Programs run their own relay sequences alongside the cross-program channel, named `{PROGRAM}-nn` (e.g. APM-nn); the XP sequence is never overloaded for program-internal traffic. Cross-program relay names carry the program token per the channel schema ruling (XP-R2).
...
— *End v1.4. Amendments by versioned revision only.*

**FIX-SURFACE-DIVERGENCE** *(named 2026-07-26, origin claims-lane).* A fix, check, or state lands on one surface while a different surface executes: the audit surface reads clean while the execution surface still runs the old bytes, the config says one mode while the session runs another, the record is corrected while the carry stack still transports the voided artifact. Every post-fix verification that reads the audited surface instead of the executing surface confirms the wrong thing. Cures are structural: verify the executing surface (observed-mode receipts, §8), generate rather than copy (§7.2), cross-check independent derivations and halt on divergence (§7.1, §7.3). THESIS MAPPING fields of future evidence entries point at this name.

**Evidence grades** *(cross-reference).* The evidence schema (XP-R1 §3, origin custody) is under revision to carry per-entry grades — RECEIPT / RECORD / ATTESTATION — for incidents predating a program's repo. Grades classify evidence strength; they never soften eligibility: an ATTESTATION-grade entry publishes only as narrative with its grade disclosed.
